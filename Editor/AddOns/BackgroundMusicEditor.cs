...
namespace DGames.Audio.Editor
{
    [CustomEditor(typeof(BackgroundMusic))]

    public class BackgroundMusicEditor : UnityEditor.Editor
    {
        private SerializedProperty _keepOnSceneLoadProperty;
        private SerializedProperty _allowedScenesProperty;

        private void OnEnable()
        {
            _keepOnSceneLoadProperty = serializedObject.FindProperty(BackgroundMusic.KEEP_SCENE_LOAD_PROPERTY);
            _allowedScenesProperty = serializedObject.FindProperty(BackgroundMusic.ALLOWED_SCENES_PROPERTY);
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.HelpBox(
                "Enable Keep On Scene Load for Uninterrupted music when scene load. Music will continue to play instead start from beginning.",
                MessageType.Info);
            EditorGUILayout.PropertyField(_keepOnSceneLoadProperty);

            if (_keepOnSceneLoadProperty.boolValue)
            {

                var editorScenes = EditorBuildSettings.scenes.ToList();
                for (int i = 0; i < _allowedScenesProperty.arraySize; i++)
                {
                    if (_allowedScenesProperty.GetArrayElementAtIndex(i).intValue > editorScenes.Count - 1)
                    {
                        _allowedScenesProperty.DeleteArrayElementAtIndex(i);
                        --i;
                    }
                }

                var allowedScenes = _allowedScenesProperty.ToValueEnumerable<int>().ToList();
                EditorGUI.indentLevel++;

                for (var i = 0; i < EditorBuildSettings.scenes.Length; i++)
                {
                    var scene = EditorBuildSettings.scenes[i];
                    var value = allowedScenes.Contains(i);
                    EditorGUI.BeginChangeCheck();
                    value = EditorGUILayout.Toggle(scene.path.Split('/').Last().Split('.').First(), value);
                    if (EditorGUI.EndChangeCheck())
                    {
                        if (value)
                        {
                            _allowedScenesProperty.InsertArrayElementAtIndex(_allowedScenesProperty.arraySize);
                            _allowedScenesProperty.GetArrayElementAtIndex(_allowedScenesProperty.arraySize - 1)
                                    .intValue =
                                i;
                        }
                        else
                        {
                            var index = _allowedScenesProperty.ToValueEnumerable<int>().ToList().IndexOf(i);
                            if (index < 0)
                                throw new Exception();
                            _allowedScenesProperty.DeleteArrayElementAtIndex(index);
                        }
                    }
                }
